...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === '/ai-status') return json({ ok: true, version: APP_VERSION, geminiConfigured: !!(env && env.GEMINI_API_KEY), workersAiConfigured: !!(env && env.AI), geminiModel: env && env.GEMINI_MODEL || GEMINI_MODEL, geminiCandidates: geminiCandidateModels(env).map(geminiModelLabel), workersAiModel: WORKERS_AI_VISION_MODEL, workersAiAgreeEndpoint: '/workers-ai-agree', features: ['ai-scan-quality','local-correction-memory','barcode-lookup','packaged-product-serving-memory','personal-nutrition-intelligence','weight-goal-tracking','smart-recommendations','meal-planning','local-ai-nutrition-coach','daily-coach-review','weekly-coach-review','coach-plan-generator','coach-main-nav','food-library-in-add','goal-based-target-engine','bmr-tdee-estimator','activity-level-targeting','goal-mode-targeting','ios-sticky-context-headers','gradient-sticky-header-material','add-sticky-header-fix'] }, 200, origin);
